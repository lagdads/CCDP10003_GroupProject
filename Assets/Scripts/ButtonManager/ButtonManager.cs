...
{
    /*自动播放区*/
    [Header("自动播放按钮")] public Image buttonImage;
    ...
    /*隐藏文本区*/
    [Header("隐藏文本按钮")] public GameObject DialogueBox;

    /*对话历史区*/
    [Header("显示历史对话按钮")] public GameObject historyPanel;
    public Transform historyItems;
    public GameObject itemPref;

    [Header("设置按钮")] public GameObject settingPanel;
    //需要获取音频管理器中的音频列表
    ...
    private readonly List<HistoryItem> itemsList = new();
    ...
    public void ButtonHistory()
    {
        ...
        foreach (HistoryItem item in itemsList)
        {
            if (item != null)
            {
                ...
                var historyLineName = newItem.transform.Find("Name").GetComponent<TMP_Text>();
                var historyLineContent = newItem.transform.Find("Content").GetComponent<TMP_Text>();
